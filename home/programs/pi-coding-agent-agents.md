...
- The built-in `read` tool only handles files, not directories. 
- The `filesystem` MCP server provides `list_directory` and `directory_tree` tools for listing directory contents — prefer these over `read` for directories.
- Fallback: use `bash` with `ls` to list a directory.
...
MCP Servers Configured (pi-coding agent):
  - context7 (url MCP)
  - filesystem (command: filesystem MCP server — provides list_directory, directory_tree, etc.)
  - mcp-nixos (url MCP)
